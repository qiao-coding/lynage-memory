// Flat FTS baseline for the 10k fair benchmark — message-level keyword search,
// NO tree, NO compression, NO semantic embedding. Mirrors the README's
// "Flat FTS uses search_messages with default top-5 truncation" claim so the
// comparison is reproducible in-repo instead of externally-sourced.
//
// Uses the SAME generated conversations and questions as bench-10k.ts (fair),
// stores raw messages, then answers via searchMessages top-5.
//
// Usage (cwd = benchmarks/baseline):
//   TURNS=2000 pnpm tsx bench-flat.ts
...
// --- same generated conversations + questions as bench-10k.ts (fair) ---
...
for (let i = 1; i <= T; i++) {
  if (fi < QN && i === factTurns[fi]) {
    ...
    const d = pk(["进度约60%核心已跑通", "遇到状态同步竞态问题", "不同尺寸下表现不一致", "大数据量渲染卡顿需虚拟滚动", "API一致性需梳理"]);
...
async function main() {
  console.log(`Flat FTS baseline: ${T} turns, ${questions.length} questions (top-5 searchMessages, no tree)`);
  const db = path.resolve(process.cwd(), "data", "flat-fair.db"); try { fs.unlinkSync(db); } catch {}
  // No model → no archiving, no tree, no compression. Messages only.
  ...
  for (let i = 0; i < questions.length; i++) {
    const q = questions[i]!;
    const s0 = performance.now(); const msgs = await mem.store.searchMessages(q.search, "s1"); tssum += performance.now() - s0;
    ...
    if ((i + 1) % 5 === 0) console.log(`  ${i + 1}/${questions.length}: ${acc}ok ${hal}hal srch=${(tssum / (i + 1)).toFixed(0)}ms in=${ti} out=${to}`);
  }
  const ansS = (performance.now() - a0) / 1000; const cost = ti * IC + to * OC;
  console.log(`\n${"=".repeat(55)}`);
  console.log(`Flat FTS results`);
  ...
  console.log(`Search: ${(tssum / questions.length).toFixed(0)}ms LLM:${(tl / questions.length).toFixed(0)}ms`);
  const out = { system: "Flat FTS", turns: T, questions: questions.length, accuracy: acc / questions.length, correct: acc, hallucination: hal, total_input: ti, total_output: to, cost_cny: cost, avg_search_ms: tssum / questions.length, avg_llm_ms: tl / questions.length, store_s: stS, answer_s: ansS, chunks: 0, dirs: 0, tree_usage_pct: 0, dirs_scanned: 0, chunks_checked: 0 };
  fs.writeFileSync(path.resolve(process.cwd(), "data", "10k_fair_flat.json"), JSON.stringify(out, null, 2));
