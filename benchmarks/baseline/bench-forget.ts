...
// Each fact is a 3-message PROCESS: try mainstream -> find problems -> pick contrarian
// [topic, mainstream(WRONG), tried, contrarian(chosen), problem, preciseDetail]
...
for(let i=1;i<=T;i++){
  if(factTurnSet.has(i)){
    ...
    if(i===factStartTurns[fiIdx]){ // turn 1: try mainstream
    ...
    }else if(i===factStartTurns[fiIdx]+1){ // turn 2: find problems
