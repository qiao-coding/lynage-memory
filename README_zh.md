...
> **摘要 + 原文高于仅摘要，说明剧情细节活在原始对白里，摘要是导航不是存储** —— Lynage 的 `openSource` 恢复原文是保真的关键。嵌入模型也关键：`bge-small-en` 读中文差（58%，噪声 ~0.74）；`bge-small-zh` 达 92%。中文叙事必须用中文/多语言嵌入。剩余的角色记忆 67% 反映检索波动（不同 run 67-100%）。
