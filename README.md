...
| 幻觉率 | **<5%** | 15-25% | 40%+ |
| Token (500轮) | **~650 (稳定)** | ~350 (信息不断丢失) | ~80 |
| 搜索延迟 | ~50ms (FTS5) | ~50ms | N/A |
| 原文可恢复 | **✅** | ❌ | ❌ |

Lynage 的 Token 消耗略高于传统上下文压缩（~650 vs ~350），但差距稳定，不随轮次拉大。多出来的 ~300 tokens 是工作记忆和阶段概述——换来的是原文可恢复、准确率高 30%、幻觉率低 15%。搜索和原文验证都是本地 SQLite 操作，毫秒级，不调 LLM。
