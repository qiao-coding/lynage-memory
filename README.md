...
消息 (不可变，追加写)
  │
  ├─ 归档 → 窗口 (摘要 + 进度 + 结论 + 目标 + sourceFrom→sourceTo 原文指针)
  │           │
...
2. **窗口 = 摘要索引，不是压缩** — 归档时 AI 读取全部原文，生成总结/进度/结论/目标。窗口保留 `sourceFrom→sourceTo` 指针指向原文，摘要仅用于搜索导航。结构化字段（结论/目标）像书的目录一样，让 AI 先判断相关性再决定打开哪个窗口。
