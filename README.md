...
| 幻觉率 | 0% | 0% |
| 输入 Token（10 题） | 2,429 | 23,564 |
| 输出 Token（10 题） | 2,156 | 1,878 |
| 输入:输出比 | 53:47 | 93:7 |
| 总 Token（10 题） | 4,585 | 25,442 |
| 搜索延迟 | **6ms** | 20ms |
...
> **10,000 轮时差异完全显现。** Lynage 准确率 100%（阶段树精度不降），Hermes 掉到 70%（平面搜索被淹没在 2 万条消息中）。Lynage 搜索 6ms（子树剪枝，只下钻相关分支），Hermes 20ms（FTS5 全量匹配）。
>
> **Token 差异的关键在输入端。** Lynage 输入 2,429 token——搜索命中后只取相关 chunk 的精简片段（`openSource` maxTokens 截断）。Hermes 输入 23,564 token——`get_messages_around` 返回命中消息 ±3 条的完整窗口，10 题累计近 9.6 倍输入。总成本 Lynage 是 Hermes 的 1/4。
