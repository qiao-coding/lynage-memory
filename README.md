...
| --- | --- | --- |
| 准确率 | **100%**（10/10） | 70%（7/10） |
| 幻觉率 | 0% | 0% |
| 搜索延迟 | **6ms** | 20ms |
| 存储时间 | 24s（异步归档，不阻塞） | 30s |
| 回答时间 | 38s | 53s |
| 回答成本 | **¥0.007** | ¥0.027 |
| 归档结构 | 38 chunks + 阶段目录 | 无（平面消息） |

> **10,000 轮时差异完全显现。** Lynage 准确率 100%（阶段树精度不降），Hermes 掉到 70%（平面搜索被淹没在 2 万条消息中）。Lynage 搜索 6ms（子树剪枝，只下钻相关分支），Hermes 20ms（FTS5 全量匹配）。成本 Lynage 是 Hermes 的 1/4——阶段摘要 + 精选原文片段，而不是全文窗口。

### 测试数据的方法论说明

> **本测试使用程序生成的模板对话**，而非真实用户对话。数据从真实 API 调用采集（token 计数、延迟、回答均为实测），但对话内容为合成。
>
> 模板数据的关键特征是**事实关键词原样出现在消息中**（如「PostgreSQL」「Docker」）。这对 Hermes 的平面 FTS5 关键词搜索是**直接利好**——关键词命中即可定位。Lynage 在此环境下经过完整归档链路（AI 摘要 + 阶段树 + 原文验证），属于「在对手的舒适区测试」，依然保持 100% 准确率。
>
> 真实对话中事实表述通常含糊（「上次那个方案」「把数据库换了」），FTS5 关键词命中率会显著下降，Hermes 的准确率预计比 70% 更低；Lynage 的 AI 摘要承担语义导航，含糊表述仍可通过摘要匹配定位。**因此该模板测试相对偏向 Hermes，Lynage 的 100% 是真实压力下的表现。**
