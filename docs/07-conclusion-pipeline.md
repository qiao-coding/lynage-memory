...
先划清界限——Lynage 的摘要和传统压缩是两回事。

```
传统压缩:                          Lynage:
                                  
  原文 ──→ 摘要 ──→ 丢弃原文        原文 ──→ 摘要标签 ──→ 原文保留
           ↑                             ↑        ↑
      摘要就是记忆                   摘要只是导航    原文才是记忆
      (丢了就没了)                  (丢了还能找回)  (永不丢失)
```

**传统压缩**：摘要生成了，原文丢了。Agent 的"记忆"就是那一段摘要文本。准确度完全依赖压缩质量。

**Lynage**：摘要只贴在窗口上当标签。Agent 搜索时先看标签，找到相关窗口后**打开原文阅读**。标签不准确也没关系——打开原文就什么都清楚了。

打个比方：
- 传统压缩 = 把一本书烧了，只留一张内容简介
- Lynage = 给每本书贴个标签，书还在书架上，随时可以取下来读
...
```
消息 (原文，永不丢失)     窗口标签              阶段标签            最终结论
────────────────────────────────────────────────────────────────────────────

msg-1: "用CSS Modules吧"                           📁 阶段: 前期开发
msg-2: "好的，CSS很适合"   → AI读全部消息 →        overallContent:     ← AI读20个标签
msg-3: "Button怎么做?"     "讨论了技术选型，          "从技术选型开始，     合成
...                        确定CSS+TS+Vite，        经历了样式迁移，
msg-42: "迁移完成"         开始组件开发"            最终确定Tailwind"    
                           │                       │
                           🗂 窗口 #1               │
                           summary: "技术选型"       │
                           keywords: [CSS, TS]      │
                           │                       │
                           sourceFrom: msg-1 ←──────┘ 标签始终保留指针
                           sourceTo:   msg-42 ←──────── 可以随时找回原文

用户提问时:
  看标签 → 找到窗口 #1 → 打开 msg-1~msg-42 原文 → 阅读 → 得出可靠结论
  而不是: 看标签 → 标签说啥就信啥 ❌
```
