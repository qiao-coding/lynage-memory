...
```
  ...
  Layer 4  Generations         窗口套窗口（G0→G1→G2）
  Layer 3  Directories         窗口目录
  Layer 2  Context Chunks      冻结的上下文窗口
...
## Layer 2：Context Chunk — 冻结的上下文窗口

当活跃窗口超过 Token 阈值，旧对话被「冻结」成一个上下文窗口。
...
**每个目录 = 一组窗口的索引**，记录了这段时间项目推进了什么、形成了什么结论。

---

## Layer 4：代际压缩 — 窗口套窗口

G0 装满 20 个窗口 → 升代：创建一个更大的窗口来概括它们。

```
之前：                         之后：
  📁 Root                       📁 Root
  └── 📁 G0 (20 个窗口)          └── 📁 G1 (摘要："架构设计阶段")
                                       └── 📁 G0 (20 个窗口，降级)
